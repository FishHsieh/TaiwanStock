...
- 2026-07-17: Fixed `^TWII` stale-close handling. `fetch_twse_taiex_snapshot()` now validates that TWSE `FMTQIK` latest row matches the report date; if it lags after close, it falls back to official `MI_5MINS_HIST` for the same-day close and `MI_INDEX type=MS` for market volume instead of reusing the prior trading day as live data.
- 2026-07-17: Rebuilt the local report in no-email/no-sync mode after the `^TWII` fix. Corrected Taiwan weighted index: `2026/07/17 42,671.27`, change `-2,953.71`, `-6.47%`, volume `18,244,071,063`. Local outputs: `Reports\Report_20260717_1741.html` and `Reports\final_article_20260717_173705.txt`. Google Workspace/Firebase were intentionally not updated in this corrective run.
- 2026-07-17: Repaired the local Python environment by repointing `.venv\pyvenv.cfg` to the available CPython 3.12 runtime and reinstalling Python dependencies for 3.12 compatibility. Added `TAIWANSTOCKBOT_SKIP_SYNC=1` support to `run_bot.ps1` for local-only reruns that must not publish to Google Workspace or Firebase.
- 2026-07-17: Removed Google Doc/Sheet from the web report and stopped the background helper from updating Google Workspace. Future publishing should deploy Firebase Hosting only; Google Sheets remains only as an input source for moving-average context unless that data source is replaced.
- 2026-07-17: Deployed corrected Firebase Hosting version `20260717_1741` to `https://vinson-stock-bot.web.app`; verified the public page has no Google Doc/Sheet links.
- 2026-07-17: Refined report wording for relative strength. The Analyzer prompt now distinguishes "price resilience" from "technical structure"; large same-day decliners must not be called `相對抗跌` merely because they still hold mid/long moving averages. Rebuilt and deployed Firebase Hosting version `20260717_1801`; conclusion now says names like 8046南電 are structurally supported but not price-resilient.
